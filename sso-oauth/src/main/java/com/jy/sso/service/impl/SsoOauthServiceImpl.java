...
import com.jy.common.properties.auth.SsoOauthProperties;
...
public class SsoOauthServiceImpl implements SsoOauthService {
    ...
    public SsoOauthToken doLogin(LoginUser loginUser) {
        ...
        ResultBean<User> userResultBean = userFeign.findUserInfo(loginUser.getAccount());
        User user = null;
        if (Objects.equals(userResultBean.getCode(), ResultBeanCode.SUCCESS.getCode())){
            user = userResultBean.getData();
        }else{
            CastException.cast(ResultBeanCode.LOGIN_ERROR);
        }
        ...
        String pwd = new BCryptPasswordEncoder().encode(loginUser.getPassword());
